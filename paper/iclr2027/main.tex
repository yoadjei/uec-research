...
objects in the recent literature (Appendix~\ref{app:terms}).
...
\label{tab:related}
\centering\small
% Fixed-width wrapped columns: with plain l columns the citation column runs 97pt past the text
% block and into the margin.
\begin{tabular}{@{}p{0.17\textwidth}p{0.42\textwidth}p{0.35\textwidth}@{}}
\toprule
...
holds. Collinearity constrains the allocation further, since no ranking can be simultaneously
faithful, stable and complete when features are collinear \citep{attribimposs2026}; our
redundant-feature block is built to exercise that case. No class is protected in its \emph{allocation} across features, which is the part a
...
\citep{rethinkrobust2025,mcal2026}. Figures are drawn to be legible under the common colour-vision
...
The phrase names at least three distinct objects in work from 2023 onwards, which is why we avoid
